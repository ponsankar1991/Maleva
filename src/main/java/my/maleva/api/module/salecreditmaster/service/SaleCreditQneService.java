...
public class SaleCreditQneService {
    ...
    /**
     * Pushes the credit note, one push at a time.
     *
    ...
    public QnePushResult push(Integer creditNoteId, Integer companyId) {
        String lockKey = "sale-credit:" + creditNoteId;
        if (!pushLock.tryAcquire(lockKey)) {
            return QnePushResult.localError(409,
                    "This credit note is already being sent to QNE and QNE has not answered yet. "
                            + "Wait for it to finish and refresh — pushing again now would "
                            + "create a second credit note in QNE.");
        }
        try {
    ...
    private QnePushResult doPush(Integer creditNoteId, Integer companyId) {
